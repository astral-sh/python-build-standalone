...
def build_docker_image(client, image_data: bytes, image_dir: pathlib.Path, name, host_platform):
...
def get_image(client, source_dir: pathlib.Path, image_dir: pathlib.Path, name, host_platform):
    if client is None:
        return None
    ...
    try:
        ...
        if tar_path.exists():
            ...
            return build_docker_image(client, str(source_dir).encode(), image_dir, name, host_platform)
